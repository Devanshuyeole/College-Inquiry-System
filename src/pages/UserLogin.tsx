...
const UserLogin = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        // ✅ FIXED: Now passing both name AND email
        login({ 
          name: data.user.name,
          email: data.user.email  // ✅ ADDED EMAIL
        });
        
        showMessage('success', isLogin ? "Login successful!" : "Account created successfully!");
        navigate("/");
      } else {
    ...
    <div className="min-h-screen flex items-center justify-center bg-muted/30 p-4">
      <Card className="w-full max-w-md">
        <CardHeader className="text-center">
          ...
          <CardDescription>
            {isLogin ? "Access your inquiries and updates" : "Register to track your inquiries and application status"}
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="text-center">
              <Button type="button" variant="link" className="text-sm" onClick={toggleMode} disabled={isLoading}>
